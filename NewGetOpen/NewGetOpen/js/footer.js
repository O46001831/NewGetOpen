...
function calcDocumentHeight() {
    ...
    var master231Height = 0; // document.getElementById('mater231').offsetHeight;
...
//_____________________  EVENTI MAPS, TELEFONO E EMAIL  _____________________
window.addEventListener('DOMContentLoaded', () => {
    // Indirizzo - Google Maps
    const address = document.getElementById('address');
    const mapsLink = document.createElement('a');
    mapsLink.href = 'https://www.google.com/maps/search/?api=1&query=Via+Cesare+Vivante+9,+Catania,+95123,+IT';
    mapsLink.target = '_blank';
    mapsLink.style.textDecoration = 'none';
    mapsLink.style.color = 'inherit';
    address.parentNode.replaceChild(mapsLink, address);
    mapsLink.appendChild(address);

    // Telefono - click to call
    const phone = document.getElementById('phoneNumber');
    const phoneLink = document.createElement('a');
    phoneLink.href = 'tel:+390958997264';
    phoneLink.style.textDecoration = 'none';
    phoneLink.style.color = 'inherit';
    phone.parentNode.replaceChild(phoneLink, phone);
    phoneLink.appendChild(phone);

    // Email - click to email
    const email = document.getElementById('emailAddress');
    const emailLink = document.createElement('a');
    emailLink.href = 'mailto:info@getopen.it';
    emailLink.style.textDecoration = 'none';
    emailLink.style.color = 'inherit';
    email.parentNode.replaceChild(emailLink, email);
